...
import ProdutoClient from './lanterna';
...
export default function ProdutoPage({ params }: Props) {
  const [lanterna, setLanterna] = useState<Product| null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchProduct() {
      try {
        const res = await fetch(`http://localhost:3001/lanternas/${params.id}`);
        if (res.ok) {
  ...
  return <ProdutoClient product={lanterna} />
}
